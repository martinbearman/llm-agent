...
import { z } from "zod";
import type { askDeepSearch } from "~/deep-search";
...
// Verdict guidelines organized by category
const VERDICT_GUIDELINES = {
...
const EVALUATION_REMINDERS = `REMEMBER: 
...
const NO_STATEMENT_EXAMPLES = `Examples of "no" statements:
...
const EVALUATION_EXAMPLE = `Example:
...
const verdictScores: Record<"yes" | "no" | "unsure", number> = {
  yes: 1,
  no: 0,
  unsure: 0.5,
};

async function checkAnswerRelevancy(opts: {
  question: string;
  submission: string;
}) {
  ...
  const scores = verdictsResult.verdicts.map(
    ({ verdict }) => verdictScores[verdict],
  );
