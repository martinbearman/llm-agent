      ...
      try {
        ...
        if (response.ok) {
          ...
          if (contentType.includes("application/pdf")) {
            try {
              const [pdfModule, arrayBuffer] = await Promise.all([
                import("pdf-parse"),
                response.arrayBuffer(),
              ]);

              const pdf = pdfModule.default ?? pdfModule;
              const pdfData = await pdf(Buffer.from(arrayBuffer), { max: 3 });
              const pdfText = (pdfData?.text ?? "").trim();

              if (!pdfText) {
                return {
                  success: false,
                  error: `Unable to extract text from PDF: ${url}`,
                };
              }
              ...
              return {
                success: false,
                error: `Failed to process PDF at ${url}: ${
...
export const crawlMultipleUrls = cacheWithRedis(
  "crawlMultipleUrls",
  async (urls: string[], options?: CrawlOptions): Promise<BulkCrawlResponse> => {
    return bulkCrawlWebsites({
      urls,
      ...(options ?? {}),
    });
  },
